    ...
    return (
        <header className={"header" + applyClassNameIfTrue(isScrolledBelowThreshold || menuIsRendered, "header--bg")}>
            <section className="header__box">
                ...
                {renderElementIfTrue(
                    menuIsTimeoutActive || isWidthAboveThreshold,
                    <nav
                        className={
                            "header__box header__box--max-size" +
                            applyClassNameIfTrue(
                                !isWidthAboveThreshold,
                                `header__box--${menuIsRendered ? "open" : "close"}`
                            )
                        }
                    >
                        <ul className="header__box header__box--list">
                            {navbarItems.map((item) => (
                                <li
                                    key={item.href}
                                    className="header__box header__box--max-size"
                                >
                                    <a
                                        key={item.href}
                                        href={`#${item.href}`}
